...
<html>
    ...
    <body>
        <table>     
            <tr>
                <th> donation_information_id</th> 
                <th> donation_type</th> 
                <th> payment_name</th>
                <th>amount</th>
                <th>number_of_unites</th>
                <th> date </th>
                <th> donatio_description </th>
                <th>reistration_id</th>       
                <th>is active</th>

                <?php
                include '../controller/connection.php';
                $query = "select * from tbl_donation_information";
                $result = mysqli_query($conn, $query);
                if ($result) {
                    while ($row = mysqli_fetch_array($result)) {
                        echo "<tr><td>$row[donation_information_id]</td>"
                        . "<td> $row[donation_type]</td>"
                        . " <td> $row[payment_name]</td>"
                        . " <td> $row[amount]</td>"
                        . " <td> $row[number_of_unites] </td> "
                        . " <td> $row[date] </td> "
                        . " <td> $row[donatio_description] </td>"
